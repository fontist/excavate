...
nested archives. Default: `false`. NOTE: Automatically set to `true` when
`files:` option is provided.
`files`:: (optional) Array of file paths to extract. If specified, only
these files are extracted. Enables `recursive_packages` automatically.
...
# Multi-level nested extraction (recursive_packages auto-enabled)
Excavate::Archive.new("bundle.zip").extract(
  files: ["level1.zip/level2.zip/file.txt"]
)
# => ["/path/to/file.txt"]
...
Parameters::
Same as `extract`. Note that `recursive_packages` is automatically enabled
when `files:` option is provided.
